...
It's notable that the resistance against hardening have led to a slow pace for bug class mitigation and an increasing interest in finding new bugs.
A Linux Kernel recently complained to me that there are 6 security researchers looking for bugs on 1 developer trying to fix and mitigate the bugs. It's all in the incentive and rewards. We see a rapid development where there are short term incentives and goals for hardware related security. Just look at how swiftly the bugs Spectre and Meltdown was addressed with the support from the architecture companies. The problem of attention and reward in software warrants it's own newsletter post.
